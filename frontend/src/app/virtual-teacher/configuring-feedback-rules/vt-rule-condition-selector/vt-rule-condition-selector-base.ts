import { ComponentRef, EventEmitter, OnDestroy, OnInit, Output } from "@angular/core";
...
import { VtConditionComponentFactoryService } from "src/app/services/virtual-teacher/configuring-rules/vt-condition-component-factory/vt-condition-component-factory.service";
...
export abstract class VtRuleConditionComponentBase implements OnInit, IVTRuleCondition, IValidatable<void> {
    ...
    validate() {
        this.isValid = this.configurableConditions?.findIndex(x => x.type == this.type) >= 0 ?? false;
    }
    ...
    constructor(protected ruleService: VtRulesService, protected componentFactory: VtConditionComponentFactoryService) {}
    ...
    type: string;
    get display(): string {
        return this.configurableConditions?.find(x => x.type == this.type)?.display ?? '';
    };
    attributes: VTRuleAttribute[];
    condition1: VTRuleCondition;
    condition2: VTRuleCondition;
    get description(): string {
        return this.configurableConditions?.find(x => x.type == this.type)?.description ?? '';
    }

    public onDelete: EventEmitter<VtRuleConditionComponentBase> = new EventEmitter<VtRuleConditionComponentBase>();
    public onWrap: EventEmitter<WrapEventArgs> = new EventEmitter<WrapEventArgs>();
    public onUnwrap: EventEmitter<UnwrapEventArgs> = new EventEmitter<UnwrapEventArgs>();
    ...
    public abstract makeCondition(): VTRuleCondition;

    public abstract setValues(condition: IVTRuleCondition);
    ...
    abstract get supportedConditions(): VTConfigurableCondition[];
...
export declare interface IConditionComponentParent extends OnDestroy {
    type: string;
