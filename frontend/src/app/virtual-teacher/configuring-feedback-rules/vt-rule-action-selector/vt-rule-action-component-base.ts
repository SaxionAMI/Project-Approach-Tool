import { AfterViewInit, ChangeDetectorRef, ComponentRef, EventEmitter, OnInit } from "@angular/core";
...
import { IVTRuleAction, VTRuleAction } from "src/app/models/virtual-teacher/rules/vt-rule-action";
import { VTRuleAttribute } from "src/app/models/virtual-teacher/rules/vt-rule-attribute";
import { VtActionComponentFactoryService } from "src/app/services/virtual-teacher/configuring-rules/vt-action-component-factory/vt-action-component-factory.service";
import { VtRulesService } from "src/app/services/virtual-teacher/configuring-rules/vt-rules-service/vt-rules.service";

export abstract class VtRuleActionComponentBase implements OnInit, IVTRuleAction, IValidatable<void>, AfterViewInit {
    constructor(protected rulesService: VtRulesService, private componentFactory: VtActionComponentFactoryService, private changeDetector: ChangeDetectorRef) {}
    ...
    validate() {
        this.isValid = 
            (this.configurableActions?.findIndex(x => x.type == this.type) >= 0 ?? false) &&
            (this.secondaryActionComponent?.instance.isValid ?? true);
    }

    abstract get viewRef(): DynamicComponentAnchor;
    ...
    public set secondaryActionComponent(value: ComponentRef<VtRuleActionComponentBase>) {
    ...
    abstract makeAction(): VTRuleAction;
    ...
    public setExistingSecondaryAction(secondaryAction: VTRuleAction) {
        this.componentFactory.makeFilledComponent(secondaryAction, this.viewRef.viewContainerRef).subscribe(x => {
    ...
    private subscribeToChildEvents(child: VtRuleActionComponentBase) {
        this.onDeleteSubscription = child.onDelete.subscribe(_ => this.deleteSecondaryAction());
